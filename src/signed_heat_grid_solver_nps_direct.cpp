#include "signed_heat_grid_solver_nps_direct.h"
...
void SignedHeatGridSolver::exportData(const Vector<double>& phi, const SignedHeat3DOptions& options) const {
    ...
    imageData->SetOrigin(bboxMin[0], bboxMin[1], bboxMin[2]);
    imageData->AllocateScalars(VTK_DOUBLE, 1);

    // Fill the grid with values
    double* ptr = static_cast<double*>(imageData->GetScalarPointer());
    int numPoints = dims[0] * dims[1] * dims[2];
    for (int i = 0; i < numPoints; ++i) {
        ptr[i] = phi[i]; // Example data
    }

    // Write the data to a VTK file (.vti)
    filename = "../export/" + options.meshname + ".vti";
